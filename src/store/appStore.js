 ...
 *
 * IMPORTANTE: todos los createEffect están a nivel de módulo (NO dentro de
 * useAppStore) para evitar duplicados. useAppStore() es un simple accessor
 * que devuelve referencias a signals y acciones ya inicializadas.
 */
...
// ═══════════════════════════════════════════════════════════════════════════════
// Acciones (module-level — NO van dentro de useAppStore)
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// Efectos reactivos (module-level — se crean UNA SOLA VEZ)
// ═══════════════════════════════════════════════════════════════════════════════
...
// Guard: no re-fetch si el tune_id no ha cambiado
let _lastFetchedTuneId = null;

createEffect(() => {
  const tune = selectedTune();
  if (!tune) {
    _lastFetchedTuneId = null;
    setTuneEntries([]);
    setActiveEntry(null);
    return;
  }

  // GUARD: si ya estamos cargando este mismo tune, no repetir
  if (tune.tune_id === _lastFetchedTuneId) return;
  _lastFetchedTuneId = tune.tune_id;
...
// ═══════════════════════════════════════════════════════════════════════════════
// Acciones adicionales (module-level)
// ═══════════════════════════════════════════════════════════════════════════════
...
const openAddFormForTune = (tune) => {
  setAddFormInitialTune(tune);
  setShowAddForm(true);
};

// ═══════════════════════════════════════════════════════════════════════════════
// Public API: useAppStore() es un simple accessor, NO crea efectos
// ═══════════════════════════════════════════════════════════════════════════════
